...
def get_video_list(args):
    ...
    train_videos = sum([v for k, v in video_split.items() if k != args.fold], []
                       ) if 'crossval' in args.split else video_split['train']
    test_videos = sum([v for k, v in video_split.items() if k == args.fold],
                      []) if 'crossval' in args.split else video_split['test']
