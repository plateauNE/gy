...
def parse_args():
    ...
    parser.add_argument('--data_dir', default="/home/imris/project/dataset/CholecT50", type=str,
                        help='data directory')
    ...
    parser.add_argument('--split', default='cholect50-challenge', type=str,
...
def train_net(net, loader, optimizers, schedulers, args, epoch, mode="train"):
    ...
    for i, (frames, y_i, y_v, y_t, y_ivt) in enumerate(tqdm_loader):
        ...
        if mode == 'train':
            ...
            loss = bce_loss(i_p, y_i) + bce_loss(v_p, y_v) + \
                bce_loss(t_p, y_t) + bce_loss(ivt_p, y_ivt)
        ...
        tqdm_loader.set_postfix(mode=mode.upper(), epoch=epoch,
                                batch=i, loss=f"{loss_tracker.avg:.3f}")
...
if __name__ == "__main__":
    ...
    print("\n\n\n训练时间：", train_time, file=open(logfile, "a+"), end="\n")
    print(">>>>>>>>>>>>>>>>>>>>>>  Args Start <<<<<<<<<<<<<<<<<<<<<<<\n", file=open(logfile, 'a+'))
    ...
    optimizer1 = torch.optim.SGD(
        params1, lr=opt_dict["opt1"]["lr"], weight_decay=decay1, momentum=mom_y)
    scheduler1 = LinearLR(
        optimizer1, start_factor=opt_dict["opt1"]["sf"], total_iters=opt_dict["opt1"]["iters"])
    scheduler2 = ExponentialLR(optimizer1, gamma=opt_dict["opt1"]["gamma"])
    sched1 = SequentialLR(optimizer1, schedulers=[scheduler1, scheduler2], milestones=[
                          opt_dict["opt1"]["iters"]+1])

    optimizer2 = torch.optim.SGD(
        params2, lr=opt_dict["opt2"]["lr"], weight_decay=decay2, momentum=mom_y)
    scheduler3 = LinearLR(
        optimizer2, start_factor=opt_dict["opt2"]["sf"], total_iters=opt_dict["opt2"]["iters"])
    scheduler4 = ExponentialLR(optimizer2, gamma=opt_dict["opt2"]["gamma"])
    sched2 = SequentialLR(optimizer2, schedulers=[scheduler3, scheduler4], milestones=[
                          opt_dict["opt2"]["iters"]+1])

    optimizer3 = torch.optim.SGD(
        params3, lr=opt_dict["opt3"]["lr"], weight_decay=decay3, momentum=mom_y)
    scheduler5 = LinearLR(
        optimizer3, start_factor=opt_dict["opt3"]["sf"], total_iters=opt_dict["opt3"]["iters"])
    scheduler6 = ExponentialLR(optimizer3, gamma=opt_dict["opt3"]["gamma"])
    sched3 = SequentialLR(optimizer3, schedulers=[scheduler5, scheduler6], milestones=[
                          opt_dict["opt3"]["iters"]+1])
    ...
    for epoch in range(args.start_epoch, args.max_epochs+1):
        ...
        # new CholecT50
        dataset = CholecT50(dataset_dir=args.data_dir,
        ...
        if val_ivtmAP > best_metric:
            ...
            exp_state = {'epoch': epoch, 'model': net.state_dict(), }
            torch.save(exp_state, save_name)
            print(f"-Checkpoint at epoch {epoch} saved at {save_name}-", file=open(logfile, 'a+'))
            no_change_val = 0

        save_name = os.path.join(args.save_dir, args.exp_name, f'last.pth')
        exp_state = {'epoch': epoch, 'model': net.state_dict(), }
        torch.save(exp_state, save_name)
        ...
        if no_change_val == args.early_stopping_patience:
            print(
                f"-- Early Stopping applied at epoch {epoch} as Validation metric did not improve --", file=open(logfile, 'a+'))
            break
